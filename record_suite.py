...
PERF_COMMAND = "perf"
PERF_LIST_FILENAME = "perf.armA54.list"
...
def main():
    ...
    console = Console()
    perfList = []
    my_functions = CDLL(LIBRARY_PATH)
    console.print(type(my_functions))
    ...
    try:
        ...
        if processid:
            ...
            os.close(r)
            my_functions.pipe_comunication(w)
            # w = os.fdopen(w, 'w')
            # print "Child writing"
    ...
    for x in range(PMU_STEPS, len(perfList), PMU_STEPS):
        ...
        cmdBench = ["sudo", PERF_COMMAND if args.sudo else PERF_COMMAND , "stat", "-e", pmuSelected, "sysbench", "cpu", "run", "--time=3"]
    ...
    for index in range(0,len(EXPERIMENTS_LIST)):
        ...
        cmdBench = ["sudo", PERF_COMMAND if args.sudo else PERF_COMMAND, "stat", "-e", ",".join(EXPERIMENTS_LIST[index]), "sysbench", "cpu", "run", "--time=3"]
