...
def drawingData():
    ...
    for OPT_STEP in OPT_STEPS:
        OPT_MAPS[str(OPT_STEP)] = []
        for avg_iter in range(0,5):
            for iteration in range(0,OPT_STEP):
                correlationoject = {}
            ...
            OPT_MAPS[str(OPT_STEP)].append(flatCorrelation)
    ...
    for idx,elm in enumerate(OPT_STEPS):
        y = [numpy.percentile(numpy.array(OPT_MAPS[str(elm)]), rang)for rang in range(0,100,2)] 
        x = [float(elm) for elm in range(0,100,2)]
        plt.plot(x, y, OPT_PLOTS[idx]['type'], label=OPT_PLOTS[idx]['label'])

    # fig, ax = plt.subplots()
    # ax.plot(x, y)

    # ax.set(ylabel='Delta', xlabel='Percentile of distribution',
    #     title='Average Delta between empirical and MVGD pairwise correlations', ylim=(0.,1.), label='no optimizzation')
    # ax.legend(loc='right')
    # ax.grid()
    ...
    #     title='', ylim=(0.,1.), facecolor='red', label='5 optimizzation steps')
    # ax2.legend(loc='right')
    # ax2.grid()
    # plt.show()

    # Plotting both the curves simultaneously
    #     plt.plot(x, y, 'b-', label='no optimizzation')
    # plt.plot(x, y, 'r--', label='5 optimizzation steps')
    plt.grid()
    
    # Naming the x-axis, y-axis and the whole graph
    plt.xlabel("Percentile of distribution")
    plt.ylabel("Delta")
    plt.title("Average Delta between empirical and MVGD pairwise correlations")
    plt.ylim(0.,1.)
...
print(df)
    ...
    # y = numpy.arange(len(a)) / float(len(a))
    # # plotting
    # plt.xlabel('x-axis')
    # plt.ylabel('y-axis')
    ...
    n, bins, patches = ax.hist(a, n_bins, density=True, cumulative=True, label='Empirical')
    # tidy up the figure
    ax.grid(True)
    ax.legend(loc='right')
    ax.set_title('Cumulative step histograms')
    ax.set_xlabel('Correlation Delta')
